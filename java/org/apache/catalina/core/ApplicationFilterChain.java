...
import com.onceas.TomcatServletHandler;
...
final class ApplicationFilterChain implements FilterChain, CometFilterChain {
	...
	private TomcatServletHandler tomcatServletHandler = new TomcatServletHandler();
	private final static Boolean wmSwitch = Boolean.valueOf(System.getProperty(
			"wm.switch", "false"));
	...
	private void internalDoFilter(final ServletRequest request,
		...
		try {
			...
			if ((request instanceof HttpServletRequest)
				...
				if (Globals.IS_SECURITY_ENABLED) {
					final ServletRequest req = request;
					final ServletResponse res = response;
					final Principal principal = ((HttpServletRequest) req)
							.getUserPrincipal();
					final Object[] args = new Object[] { req, res };

					if (wmSwitch) {
						RequestContext requestContext = tomcatServletHandler
								.getCurentRequestContext((Servlet) servlet,
										(HttpServletRequest) args[0]);
						RequestHandleWork rhw = new RequestHandleWork(
								requestContext) {
						...
						tomcatServletHandler.execute(requestContext, rhw);
					} else {
...
//						 args = null;
					...
					if (servlet instanceof DefaultServlet) {
						...
						if (wmSwitch) {
							RequestContext requestContext = tomcatServletHandler
									.getCurentRequestContext(servlet,
											(HttpServletRequest) request);
							// create the wrapper work
							RequestHandleWork rhw = new RequestHandleWork(
									requestContext) {
								public Object executeRequest() {
									try {
