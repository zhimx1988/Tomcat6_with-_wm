...
public class KalmanOneCalculate {

	public static final double DEFAULT_VALUE = 0.0001d;
	...
	public KalmanOneCalculate(double[] x) {
		this(x, QCalculate.caculate(x));
	}

	public KalmanOneCalculate(double[] x, double[][] p) {
		super();
		this.n = x.length;
		this.X = new Matrix(x, x.length); // x.length == rows
		this.P = new Matrix(p);

		meanX = new double[x.length];
		count = 0;
	}
...
//		System.out.println();
	...
	public double getMeanX(int pos) {
		return meanX[pos];
	}
